...
class IctihatAramaScreen extends StatefulWidget {
  final String title;

  const IctihatAramaScreen({
    super.key,
    required this.title,
  });

  @override
  State<IctihatAramaScreen> createState() => _IctihatAramaScreenState();
}

class _IctihatAramaScreenState extends State<IctihatAramaScreen> {
  ...
  Future<void> _aramaYap() async {
    ...
    try {
      final result = await ApiService.aramaIctihat(
        query: _searchController.text,
        tur: widget.title.toLowerCase().replaceAll(' ', '_'),
      );
                                ...
                                children: [
                                  ...
                                  Row(
                                    children: [
                                      ...
                                      Icon(Icons.description, size: 14, color: Colors.white.withOpacity(0.7)),
                                      const SizedBox(width: 4),
                                      Text(
                                        'Esas: ${sonuc['esasNo'] ?? ''}',
                                        style: TextStyle(
